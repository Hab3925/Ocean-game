...
public class WaveGeometry : MonoBehaviour
{
    [SerializeField]
    Transform viewer;
    [SerializeField]
    ...
    Mesh CreatePlaneMesh(int width, int height, float lengthScale, Seams seams = Seams.None, int trianglesShift = 0)
    {
        ...
        // Loops through the x and the y coordinate and generates vertecies for the mesh
        for (int i = 0; i < height + 1; i++)
        {
            for (int j = 0; j < width + 1; j++)
            {
                ...
                // Makes sure coordinates of the outside vertecies have cooridnates divisable by 2 if the input seam sais so? 
        ...
        // Defines what vertecies makes up each triangle of the mesh. 
        int tris = 0;
